...
Security dependency audits apply the same fail-open change detection per ecosystem: JavaScript, Rust, and Python audits skip setup for pull requests that do not change that ecosystem's manifests, lockfiles, audit allowlist, or shared tool configuration. Pushes, scheduled scans, and uncertain diffs always audit normally.
