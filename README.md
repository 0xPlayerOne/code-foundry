...
3. Add the repository's standard scripts (`format:check`, `lint`, `type-check`, `build`, and `test`/`test:coverage`) when available. The shared scripts skip checks that do not apply to the repository's languages or package manager.
