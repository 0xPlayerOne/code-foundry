...
import {
  ...
  assetManifest,
  integrityCommand,
  verifyImmutableSetting,
  verifyRelease,
} from '../src/commands/release-integrity.mjs'
  ...
  const run = (argv) => {
    ...
    if (argv[0] === 'api') {
      if (endpoint.endsWith('immutable-releases')) value = { enabled: true }
        ...
        value = { immutable: true, draft: false, tag_name: 'v1.0.0' }
    ...
    return {
      status: Object.hasOwn(overrides, 'status') ? overrides.status : 0,
      stdout: overrides.invalidJson ? 'bad' : JSON.stringify(value),
    }
  }
  return { run, calls }
}
...
test('settings verifies enabled without changing repository settings', () => {
  ...
  assert.deepEqual(fake.calls, [
    ['api', '--hostname', 'github.com', 'repos/owner/repo/immutable-releases'],
  ])
})
...
test('disabled and malformed settings cannot pass', () => {
  assert.throws(() =>
    verifyImmutableSetting('owner/repo', runner({ value: () => ({ enabled: false }) }).run)
  )
...
test('metadata alone is insufficient; signed release verification is invoked', () => {
  ...
  const report = verifyRelease(
    { repository: 'owner/repo', tag: 'v1.0.0', expectedSha: sha },
    fake.run
  )
...
for (const metadata of [{ immutable: false }, { draft: true }, { tag_name: 'other' }]) {
  test(`incorrect release metadata fails: ${JSON.stringify(metadata)}`, () => {
    const fake = runner({
      value: (argv, value) =>
        argv.at(-1).includes('/releases/tags/') ? { ...value, ...metadata } : value,
    })
...
test('tag source mismatch fails', () => {
  assert.throws(
    () =>
      verifyRelease(
        { repository: 'owner/repo', tag: 'v1.0.0', expectedSha: 'b'.repeat(40) },
        runner().run
      ),
    /source SHA/
  )
...
test('annotated tags resolve to their commit with bounded recursion', () => {
  const fake = runner()
  const run = (argv) =>
    argv.at(-1).includes('/git/ref/')
      ? { status: 0, stdout: JSON.stringify({ object: { type: 'tag', sha } }) }
      : argv.at(-1).includes('/git/tags/')
        ? { status: 0, stdout: JSON.stringify({ object: { type: 'commit', sha } }) }
        : fake.run(argv)
...
test('local asset digest is checked by the signed release verifier', (t) => {
  ...
  const fake = runner()
  const report = verifyRelease(
    { repository: 'owner/repo', tag: 'v1.0.0', root, assets: ['dist/app.tgz'] },
    fake.run
  )
  assert.equal(report.assets[0].status, 'verified')
  assert.ok(
    fake.calls.some(
      (argv) => argv[1] === 'verify-asset' && argv[3] === assetDigest(root, 'dist/app.tgz').path
    )
  )
})
...
test('unsafe, empty, or duplicate assets fail', (t) => {
  ...
  for (const file of ['../outside', '/etc/hosts', 'dist/escape', 'dist/empty', 'dist'])
    assert.throws(() => assetDigest(root, file))
...
test('CLI rejects ambiguous or ignored flags and option injection', () => {
  for (const args of [
    [],
    ['settings', '--repo'],
    ['settings', '--repo', 'owner/repo', '--tag', 'ignored'],
    ['release', '--repo', 'owner/repo', '--tag', '--help'],
    ['settings', '--repo', 'owner/repo', '--repo', 'other/repo'],
  ])
...
test('workflow and action do not grant write access to release verification', () => {
  ...
  const action = readFileSync(
    new URL('../.github/actions/attest-artifact/action.yml', import.meta.url),
    'utf8'
  )
