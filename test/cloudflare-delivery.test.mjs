...
import { deploymentVersions, digestBuild, httpsUrl, inspectBindings, parseArgv, parseUpload, pinnedWrangler, validateRollback } from '../src/lib/cloudflare-delivery.mjs'
import { deliveryCommand, latestDeployment, requestJson } from '../src/commands/cloudflare-delivery.mjs'
...
const upload = { type: 'version-upload', version: 1, worker_name: 'site', version_id: candidate, preview_url: 'https://123-site.team.workers.dev/' }
...
function environment(root) {
  return { FOUNDRY_STATE_FILE: join(root, 'state.json'), GITHUB_SHA: sha, FOUNDRY_WORKER: 'site',
    GITHUB_REPOSITORY: 'test/repo', GITHUB_REF: 'refs/heads/main', FOUNDRY_DEFAULT_BRANCH: 'main',
    GH_TOKEN: 'fixture-github-token', CLOUDFLARE_API_TOKEN: 'fixture-cloudflare-token', CLOUDFLARE_ACCOUNT_ID: 'account' }
}

function state(root, extra = {}) {
  writeFileSync(join(root, 'state.json'), JSON.stringify({ sourceSha: sha, worker: 'site', versionId: candidate,
    url: upload.preview_url, candidateVerified: true, bindingInspection: inspectBindings(version(), policy, 'production'), ...extra }))
}

function respond(value) { return new Response(JSON.stringify(value), { status: 200 }) }

test('structured output yields exact candidate identity and URL', () => {
  const text = `${JSON.stringify({ type: 'wrangler-session' })}\n${JSON.stringify(upload)}\n`
  assert.deepEqual(parseUpload(text, 'site'), { worker: 'site', versionId: candidate, url: upload.preview_url })
})

for (const data of [
  { ...upload, preview_url: '' }, { ...upload, worker_name: 'other' },
  { ...upload, version_id: 'latest' }, { ...upload, version: 2 },
  { ...upload, preview_url: 'https://evil.example/' },
]) {
  test(`invalid upload fails: ${JSON.stringify(data)}`, () => assert.throws(() => parseUpload(JSON.stringify(data), 'site')))
}
...
for (const value of ['latest', '^4.0.0', '4', '4.0.0; echo unsafe']) {
  test(`mutable Wrangler selection rejected: ${value}`, () => assert.throws(() => pinnedWrangler(value)))
}
...
test('local Wrangler never implicitly installs and exact versions are argv', () => {
  ...
  assert.deepEqual(parseArgv('["node","probe.mjs","two words; $(echo test)"]', 'probe'), ['node', 'probe.mjs', 'two words; $(echo test)'])
...
test('unreviewed state and Durable Objects fail closed', () => {
  assert.throws(() => inspectBindings(version([{ name: 'DB', type: 'd1' }]), policy, 'preview'), /Unreviewed/)
  assert.throws(() => inspectBindings(version([{ name: 'DO', type: 'durable_object_namespace' }]), policy, 'preview'), /Durable/)
  assert.throws(() => inspectBindings({ resources: { bindings: [], script_runtime: { exports: { Object: { type: 'durable-object' } } } } }, policy, 'preview'), /Durable/)
  assert.throws(() => inspectBindings({}, policy, 'preview'), /metadata/)
})

test('reviewed stateful tests never enable automatic rollback', () => {
  const inspected = inspectBindings(version([{ name: 'DB', type: 'd1' }]), { ...policy, readOnlyBindings: [{ name: 'DB', type: 'd1' }] }, 'production')
...
test('isolated previews verify resource IDs and cannot be promoted', () => {
  ...
  const isolated = { ...policy, isolatedBindings: [{ name: 'DB', type: 'd1', expected: { id: 'preview-id' }, production: { id: 'production-id' } }] }
  assert.equal(inspectBindings(version([binding]), isolated, 'preview').bindings[0].policy, 'isolated')
  assert.throws(() => inspectBindings(version([binding]), isolated, 'production'), /never be promoted/)
  assert.throws(() => inspectBindings(version([{ ...binding, id: 'production-id' }]), isolated, 'preview'), /mismatch/)
})
...
test('canary routing preserves exact baseline and total percentage', () => {
  assert.deepEqual(deploymentVersions(candidate, 10, [{ version_id: baseline, percentage: 100 }]), [
    { version_id: candidate, percentage: 10 }, { version_id: baseline, percentage: 90 },
  ])
  assert.throws(() => deploymentVersions(candidate, 10, []))
  for (const value of [0, 101, NaN, 0.001, 99.999]) assert.throws(() => deploymentVersions(candidate, value, []))
...
test('deployment ordering uses timestamps rather than trusting array order', () => {
  assert.equal(latestDeployment({ deployments: [
    { id: 'old', created_on: '2026-01-01T00:00:00Z' }, { id: 'new', created_on: '2026-01-02T00:00:00Z' },
  ] }).id, 'new')
  assert.throws(() => latestDeployment({ deployments: [{}] }))
})

test('HTTP API failures do not disclose authorization or response bodies', async () => {
  await assert.rejects(requestJson('https://api.example', 'sensitive-token', undefined,
    async () => new Response('sensitive-response', { status: 403 })), (error) => {
...
test('promotion activates the verified UUID without executing a build', async (t) => {
  ...
  t.mock.method(globalThis, 'fetch', async (url, options) => {
    ...
    if (options.method === 'GET') return respond({ success: true, result: { deployments: [
      { id: baseline, created_on: '2026-01-01T00:00:00Z', versions: [{ version_id: baseline, percentage: 100 }] },
    ] } })
  ...
  assert.equal(JSON.parse(readFileSync(join(root, 'state.json'), 'utf8')).cloudflareDeploymentId, deployed)
})

test('rollback cannot undo another concurrent deployment', async (t) => {
  const root = fixture(t)
  state(root, { cloudflareDeploymentId: deployed, previous: [{ version_id: baseline, percentage: 100 }] })
  t.mock.method(globalThis, 'fetch', async () => respond({ success: true, result: { deployments: [
    { id: candidate, created_on: '2026-01-02T00:00:00Z' },
  ] } }))
  await assert.rejects(deliveryCommand('rollback', { ...environment(root), AUTO_ROLLBACK: 'true' }), /another rollout/)
})
...
test('source mismatch and unverified candidates cannot initialize production', async (t) => {
  ...
  await assert.rejects(deliveryCommand('init-production', { ...environment(root), CANDIDATE_VERIFIED: 'true', CANDIDATE_SOURCE_SHA: 'b'.repeat(40) }), /SHA mismatch/)
})

test('workflow keeps approvals, identities, and secrets separated', () => {
  const yaml = readFileSync(new URL('../.github/workflows/cloudflare-delivery.yml', import.meta.url), 'utf8')
  assert.match(yaml, /cancel-in-progress: false/)
  assert.match(yaml, /name: \$\{\{ inputs.production-environment \}\}/)
