...
import { browserFiles, checkPackage, checkStaticSite, checkWorker, ownedPath, readTags, runProductQuality, runQualityCommand, validateBrowserReport } from '../src/lib/product-quality.mjs'
...
function site(t) {
  const root = temp(t)
  const html = (path, body) => `<!doctype html><html><head><title>Page</title><meta content="Text > other" name="description"><link href="https://example.test${path}" rel="canonical"></head><body>${body}</body></html>`
  put(root, 'dist/index.html', html('/', '<a href="about/#section">About</a><script src="/app.js"></script><img src="/a.png"><img srcset="/a.png 1x, /b.png 2x">'))
  put(root, 'dist/about/index.html', html('/about/', '<h1 id="section">About</h1><a href="../">Home</a>'))
  ...
  put(root, 'dist/sitemap.xml', '<urlset><url><loc>https://example.test/</loc></url><url><loc>https://example.test/about/</loc></url></urlset>')
  put(root, 'dist/robots.txt', 'User-agent: *\nAllow: /\nSitemap: https://example.test/sitemap.xml\n')
  put(root, 'dist/_redirects', '/old /about/ 301\n')
  const profile = { id: 'site', type: 'static-site', origin: 'https://example.test', dist: 'dist', routes: [{ path: '/', html: 'index.html', javascriptAssets: ['/chunk.js'] }, { path: '/about/', html: 'about/index.html' }], budgets: { htmlBytes: 2000, javascriptBytes: 100, imageBytes: 100 }, sitemap: 'sitemap.xml', redirects: { file: '_redirects', rules: [{ from: '/old', to: '/about/', status: 301 }] } }
  return { root, profile }
}
...
test('generated HTML ignores comment and raw-script fake metadata and preserves quoted >', () => {
  const { tags } = readTags('<!-- <meta name="fake"> --><script>const text = "<meta name=\'fake\'>"</script><meta name="description" content="a > b &amp; c">')
...
function worker() {
  return { files: ['worker.js'], buildCommand: [process.execPath, '-e', 'require("node:fs").writeFileSync(process.env.CODE_FOUNDRY_QUALITY_OUTPUT_DIR + "/worker.js", "export default {}")'], runtimeCommand: [process.execPath, '-e', 'process.exit(0)'], budgets: { rawBytes: 100, gzipBytes: 100 } }
}
...
test('quality commands do not shell-expand arguments and propagate failures', (t) => {
  const root = temp(t)
  assert.match(runQualityCommand([process.execPath, '-e', 'console.log(process.argv[1])', '$HOME; echo injected'], root, root), /\$HOME; echo injected/)
  assert.throws(() => runQualityCommand('echo hi', root, root), /argv/)
  assert.throws(() => runQualityCommand([process.execPath, '-e', 'process.exit(2)'], root, root), /failed/)
})

test('package profile installs and imports the actual tarball offline without lifecycle scripts', (t) => {
  const root = temp(t)
  put(root, 'package.json', JSON.stringify({ name: 'foundry-fixture-package', version: '1.0.0', type: 'module', exports: './index.mjs', files: ['index.mjs'], scripts: { prepack: 'exit 88', postinstall: 'exit 88' } }))
  ...
  assert.throws(() => checkPackage(root, { imports: ['node:fs'] }, root), /candidate package exports/)
  assert.throws(() => checkPackage(root, { imports: ['foundry-fixture-package'], runtimes: [] }, root), /nonempty/)
})

test('browser profiles retain reviewed baselines and require real journey modules', (t) => {
  const root = temp(t)
  put(root, 'journeys.mjs', 'export {}\n')
  const profile = { baseURL: 'http://localhost:4321', journeys: 'journeys.mjs', routes: [{ id: 'home', path: '/', readySelector: 'h1', visual: true }] }
  ...
  assert.throws(() => browserFiles(root, { ...profile, baseURL: 'https://example.test' }, root), /allowRemote/)
  assert.throws(() => browserFiles(root, { ...profile, routes: [{ ...profile.routes[0], path: '//example.test/' }] }, root), /selected origin/)
  assert.throws(() => browserFiles(root, { ...profile, routes: [...profile.routes, ...profile.routes] }, root), /Duplicate/)
})
...
test('manifest runner creates fresh failure evidence and does not treat inapplicable work as success', (t) => {
  ...
  const reports = runs.map((name) => JSON.parse(readFileSync(join(root, '.code-foundry/quality', name, 'summary.json'))))
  ...
  assert.throws(() => runProductQuality(root, { schema_version: 1, profiles: [{ id: 'bad', type: 'unknown' }] }), /Unknown quality/)
})


test('browser evidence rejects absent routes, expected failures and empty results', () => {
  const spec = (title) => ({ title, ok: true, tests: [{ expectedStatus: 'passed', status: 'expected', results: [{ status: 'passed' }] }] })
  const report = { stats: { unexpected: 0, flaky: 0, skipped: 0, expected: 2 }, suites: [{ specs: [spec('quality: home'), spec('real checkout journey')] }] }
