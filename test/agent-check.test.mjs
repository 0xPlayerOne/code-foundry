...
import { checkRepository, parseAgentArgs, planChecks, sourceContext } from '../src/commands/agent-check.mjs'
...
function fixture(t, { failure = '', applicable = ['build', 'unit'], config = '' } = {}) {
  ...
  writeFileSync(join(root, 'package.json'), JSON.stringify({ scripts: { build: 'fake', 'test:unit': 'fake' } }))
  writeFileSync(join(root, '.gitignore'), '.code-foundry/\n')
  const core = join(root, 'core.mjs')
  const runtime = join(root, 'runtime.mjs')
  writeFileSync(core, `if (process.argv[3] === 'task_profile') console.log('applicable=' + ${JSON.stringify(applicable)}.includes(process.argv[4])); else { console.log('PROJECT_STDOUT'); console.error('PROJECT_STDERR'); process.exit(process.argv[3] === ${JSON.stringify(failure)} ? 7 : 0) }`)
  const wrapper = new URL('../src/runtime.mjs', import.meta.url).href
  writeFileSync(runtime, `import { runRuntime } from ${JSON.stringify(wrapper)}; process.exitCode = runRuntime(process.argv.slice(2), process.cwd(), ${JSON.stringify(core)})`)
  ...
  return { root, paths: { core, runtime }, git, options: parseAgentArgs(['check', '--target', root, '--json']) }
}

for (const args of [[], ['other'], ['plan', '--tier', 'release'], ['check', '--timeout', '0'], ['check', '--timeout', '1.5'],
  ['plan', '--base', 'main'], ['plan', '--timeout', '10'], ['check', '--json', '--json'], ['check', '--target'], ['plan', '--unknown']]) {
  test(`invalid agent CLI input fails: ${JSON.stringify(args)}`, () => assert.throws(() => parseAgentArgs(args)))
}
...
test('plan is read-only and discovers all requirements even for a fast subset', (t) => {
  ...
  assert.equal(plan.tasks.find((task) => task.task === 'e2e').selected, false)
  assert.equal(plan.tasks.find((task) => task.task === 'performance').selected, true)
...
test('changed plan includes staged, unstaged, and untracked paths without pruning tasks', (t) => {
  ...
  writeFileSync(join(root, 'staged.txt'), 'staged'); git('add', 'staged.txt')
...
test('successful checks preserve fresh shared runtime receipts', (t) => {
  ...
  assert.equal(report.tasks.filter((task) => task.status === 'passed').length, 2)
  assert.ok(existsSync(report.report))
  for (const task of report.tasks.filter((task) => task.receipt)) {
    const receipt = JSON.parse(readFileSync(task.receipt, 'utf8'))
    assert.equal(receipt.task, task.task)
...
test('invalid revision cannot be mistaken for an empty change set', (t) => {
  ...
  options.changed = true; options.base = 'nonexistent-branch'
...
test('execution output stays on stderr while JSON remains parseable', (t) => {
  ...
  const result = spawnSync(process.execPath, ['--input-type=module', '-e',
    `import { checkRepository } from ${JSON.stringify(module)}; console.log(JSON.stringify(checkRepository(${JSON.stringify(options)}, ${JSON.stringify(paths)})))`], { cwd: root, encoding: 'utf8' })
...
test('public CLI dispatches plan help and emits structured argument errors', () => {
  ...
  assert.match(help.stdout, /code-foundry plan/)
  const invalid = spawnSync(process.execPath, [cli.pathname, 'check', '--json', '--tier', 'typo'], { encoding: 'utf8' })
