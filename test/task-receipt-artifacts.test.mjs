...
for (const [file, tasks] of Object.entries(workflows)) {
  ...
  for (const task of tasks) {
    test(`${file}: ${task} retains exactly its own hidden receipt after execution or explicit skip`, () => {
      ...
      const retention = block.split('      - name: Retain task receipt\n')[1]
        .split('      - name:')[0]
      assert.ok(retention)
      ...
      assert.ok(retention.includes(`name: \${{ inputs.artifact-prefix }}-\${{ github.run_id }}-\${{ github.run_attempt }}-${task}\n`))
      ...
      const execution = block.slice(block.indexOf('        id: execute'), block.indexOf('      - name: Retain task receipt'))
...
function fixture(t, script) {
  ...
  writeFileSync(join(directory, '.github/code-foundry.yml'), 'package_manager: npm\ncoverage_enforcement: off\n')
  writeFileSync(join(directory, 'package.json'), JSON.stringify({
    name: 'receipt-fixture', private: true, packageManager: 'npm@10.9.2',
    scripts: script ? { 'test:unit': script } : {},
  }))
  writeFileSync(join(directory, 'package-lock.json'), '{"lockfileVersion":3}')
  const env = { ...process.env, CI: 'true', GITHUB_OUTPUT: '', GITHUB_STEP_SUMMARY: '',
    REPO_FOUNDRY_PROFILE: '', REPO_FOUNDRY_LANGUAGES: '', REPO_FOUNDRY_PACKAGE_MANAGER: 'npm' }
...
for (const [name, script, expected] of [
  ...
  test(`actual runtime produces the uploaded unit receipt on ${name}`, t => {
    ...
    const report = JSON.parse(readFileSync(join(directory, '.code-foundry/results/unit.json'), 'utf8'))
