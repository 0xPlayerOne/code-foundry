...
const profile = release.match(/node <<'NODE'\n([\s\S]*?)\n          NODE/)[1]
  .split('\n').map(line => line.slice(10)).join('\n')
const identity = caller.match(/node --input-type=module <<'NODE'\n([\s\S]*?)\n          NODE/)[1]
  .split('\n').map(line => line.slice(10)).join('\n')
...
function fixture(t) {
  ...
  writeFileSync(join(directory, '.github/code-foundry.yml'),
...
function executeProfile(directory, output, env = {}) {
  return spawnSync(process.execPath, ['-e', profile], {
    cwd: directory, encoding: 'utf8', env: { ...process.env, GITHUB_OUTPUT: output,
      RELEASE_CONFIG_FILE: '', DEFER_PUBLICATION: 'false', ...env },
  })
}

const deferred = { RELEASE_CONFIG_FILE: '.github/release-please-foundry.json', DEFER_PUBLICATION: 'true' }
...
for (const [name, mutate] of [
  ['non-draft', value => { value.packages['.'].draft = false }],
  ['lazy tag', value => { value.packages['.']['force-tag-creation'] = false }],
  ['multiple packages', value => { value.packages.extra = {} }],
  ['no root package', value => { value.packages = { other: {} } }],
]) {
...
for (const path of ['../other.json', '/tmp/other.json', 'bad\npath.json', '.github/missing.json']) {
  test(`configuration rejects unsafe or missing path ${JSON.stringify(path)}`, t => {
    const { directory, output } = fixture(t)
    assert.notEqual(executeProfile(directory, output, { ...deferred, RELEASE_CONFIG_FILE: path }).status, 0)
  })
}
test('configuration cannot escape the checkout through a symlink', t => {
  ...
  const result = executeProfile(directory, output, { ...deferred, RELEASE_CONFIG_FILE: '.github/escape.json' })
  assert.notEqual(result.status, 0)
...
function identityEnv() {
  ...
  return { ...process.env, SOURCE_SHA: 'a'.repeat(40), GITHUB_SHA: 'a'.repeat(40),
    RELEASE_SHA: 'a'.repeat(40), RELEASE_TAG: `v${version}`, PACKAGE: `code-foundry-${version}.tgz`,
    CANDIDATE_SHA256: 'b'.repeat(64), GITHUB_RUN_ID: '12', GITHUB_RUN_ATTEMPT: '3',
    CANDIDATE_ARTIFACT: 'qualification-candidate-12-3' }
}
function executeIdentity(env) {
  return spawnSync(process.execPath, ['--input-type=module', '-e', identity], { cwd: root, env, encoding: 'utf8' })
}
...
for (const [key, value] of [
  ['SOURCE_SHA', 'main'], ['GITHUB_SHA', 'c'.repeat(40)], ['RELEASE_SHA', 'c'.repeat(40)],
  ['RELEASE_TAG', 'v0.0.0'], ['PACKAGE', '../unsafe.tgz'], ['CANDIDATE_SHA256', ''],
...
test('Release Please outputs preserve the exact SHA for both credential routes', () => {
  const normalize = release.slice(release.indexOf('      - name: Normalize Release Please outputs'), release.indexOf('      - name: Normalize generated release PR draft state'))
  const shell = normalize.split('        run: |\n')[1].split('\n').map(line => line.slice(10)).join('\n')
  for (const mode of ['AUTOMATION', 'WORKFLOW']) {
    ...
    try {
      ...
      for (const prefix of ['AUTOMATION', 'WORKFLOW'])
        for (const key of ['RELEASE_CREATED', 'TAG_NAME', 'SHA', 'PRS_CREATED']) env[`${prefix}_${key}`] = ''
      Object.assign(env, { [`${mode}_RELEASE_CREATED`]: 'true', [`${mode}_TAG_NAME`]: 'v1.0.0', [`${mode}_SHA`]: 'a'.repeat(40) })
...
test('pause override is explicit, manual-only and never permits non-main publication', () => {
  assert.equal([...publisher.matchAll(/github\.event_name == 'workflow_dispatch' && inputs\['billing-pause-bypass'\] == true/g)].length, 2)
