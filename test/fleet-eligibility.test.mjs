...
import {
  assertFleetEligibility,
  guardedFleetUpgrade,
  RELEASE_POLICY_FILE,
  validateReleasePolicy,
} from '../src/lib/fleet-eligibility.mjs'

const sha = 'a'.repeat(40)
const policy = {
  schema_version: 1,
  repository: 'owner/foundry',
  workflow: '.github/workflows/qualification.yml',
  branch: 'main',
  required_jobs: ['Node 20', 'Node 22', 'Node 24'],
}
...
function evidence(overrides = {}) {
  ...
  const jobs = policy.required_jobs.map((name) => ({
    name,
    status: 'completed',
    conclusion: 'success',
  }))
  ...
  const run = (command, args) => {
    ...
    if (command === 'git')
      return args.includes('rev-parse') ? sha : overrides.dirty ? ' M src/runtime.mjs' : ''
    ...
    if (endpoint.includes('/jobs?'))
      return JSON.stringify([{ total_count: 3, jobs: overrides.jobs ?? jobs }])
    if (endpoint.includes('/runs?'))
      return JSON.stringify([
        { total_count: overrides.total ?? 1, workflow_runs: overrides.runs ?? [runInfo] },
      ])
    if (endpoint.endsWith(`/actions/runs/${runInfo.id}`))
      return JSON.stringify({ ...runInfo, ...overrides.latest })
...
test('unconfigured roots retain legacy behavior without network access', (t) => {
  ...
  assert.equal(
    assertFleetEligibility(root, root, 'unused', () => {
      throw Error('unexpected call')
    }),
    null
  )
  assert.equal(
    guardedFleetUpgrade(root, root, 'unused', () => 42),
    42
  )
})
test('qualified release and all current-attempt jobs authorize the upgrade once', (t) => {
  ...
  let operations = 0
  assert.equal(
    guardedFleetUpgrade(root, root, 'v1.2.3', () => ++operations, run),
    1
  )
...
for (const [name, overrides] of [
  ...
  test(`reject ${name} before any upgrade callback`, (t) => {
    ...
    let writes = 0
    assert.throws(() =>
      guardedFleetUpgrade(root, root, 'v1.2.3', () => ++writes, evidence(overrides).run)
    )
...
test('skipped required jobs are not successes', (t) => {
  ...
  assert.throws(
    () => assertFleetEligibility(root, root, 'v1.2.3', evidence({ jobs }).run),
    /did not pass/
  )
})
  ...
  assert.throws(
    () =>
      assertFleetEligibility(
        ...
        root,
        'v1.2.3',
        evidence({ runs: [runInfo, newer], total: 2 }).run
      ),
    /Latest/
  )
})
...
test('source movement during verification invalidates eligibility', (t) => {
  ...
  let heads = 0
  const run = (command, args) =>
    command === 'git' && args.includes('rev-parse') && ++heads > 1
      ? 'b'.repeat(40)
