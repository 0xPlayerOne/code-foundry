...
import { inspectQueueCaller, mergeQueueEnabled, QUEUE_CALLER, queueRuntimeRef, renderMergeQueueCaller, syncQueueCaller, validateMergeGroup } from '../src/lib/merge-queue.mjs'
...
function event(branch = 'main') {
  const payload = { action: 'checks_requested', repository: { full_name: 'owner/app' }, merge_group: { head_sha: sourceSha, base_sha: 'b'.repeat(40), base_ref: `refs/heads/${branch}`, head_ref: `refs/heads/gh-readonly-queue/${branch}/pr-12-abcd` } }
  const environment = { GITHUB_EVENT_NAME: 'merge_group', GITHUB_SHA: sourceSha, GITHUB_REF: payload.merge_group.head_ref, GITHUB_REPOSITORY: 'owner/app' }
...
test('generated caller runs a pinned full audit with the canonical aggregate name', () => {
  ...
  assert.doesNotMatch(yaml, /pull_request:|push:|secrets:|id-token:|environment:|release_diff|mode: fast|mode: release/)
...
test('staging is supported but still receives the full audit and respects CodeQL policy', () => {
  const yaml = renderMergeQueueCaller({ ...config, git_workflow: 'staging-release', codeql: 'false', ci_runner: 'custom-ci' }, sourceSha)
...
test('unsafe or incomplete queue configuration fails before generation', () => {
  ...
  assert.throws(() => renderMergeQueueCaller({ ...config, runtime_repository: '../repo' }, 'v1.0.0'))
  assert.throws(() => renderMergeQueueCaller({ ...config, runner: '${{ secrets.TOKEN }}' }, 'v1.0.0'))
  assert.throws(() => renderMergeQueueCaller({ ...config, features: 'release' }, 'v1.0.0'), /canonical PR/)
  assert.throws(() => renderMergeQueueCaller({ ...config, codeql_rust_shards: '[]' }, 'v1.0.0'))
  assert.throws(() => renderMergeQueueCaller({ ...config, codeql_rust_max_parallel: '0' }, 'v1.0.0'))
})

for (const branch of ['main', 'staging']) test(`valid ${branch} merge groups validate their exact combined commit`, () => {
  const { payload, environment } = event(branch)
  assert.deepEqual(validateMergeGroup(payload, environment, ['main', 'staging']), { mode: 'audit', head_sha: sourceSha, base_sha: 'b'.repeat(40), base_ref: `refs/heads/${branch}` })
})

for (const [name, mutate] of [
  ['ordinary push', (p, e) => { e.GITHUB_EVENT_NAME = 'push' }],
  ['destroyed group', (p) => { p.action = 'destroyed' }],
  ['wrong head commit', (p, e) => { e.GITHUB_SHA = 'c'.repeat(40) }],
  ['wrong ref', (p, e) => { e.GITHUB_REF = 'refs/heads/main' }],
  ['missing base SHA', (p) => { delete p.merge_group.base_sha }],
  ['different repository', (p) => { p.repository.full_name = 'other/app' }],
  ['forged queue prefix', (p, e) => { p.merge_group.head_ref = e.GITHUB_REF = 'refs/heads/gh-readonly-queue/main-evil/pr-12' }],
]) test(`reject ${name}`, () => {
...
test('user-owned callers and symlinks are never overwritten', (t) => {
  ...
  assert.deepEqual(syncQueueCaller(root, null), [])
  assert.throws(() => inspectQueueCaller(root, renderMergeQueueCaller(config, 'v1.0.0')), /repository-owned/)
