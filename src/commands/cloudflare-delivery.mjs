...
import { bindingPolicy, deploymentVersions, digestBuild, httpsUrl, inspectBindings, parseArgv, parseUpload, pinnedWrangler, validateRollback, versionId } from '../lib/cloudflare-delivery.mjs'
...
export async function requestJson(url, token, body, request = fetch) {
  const response = await request(url, {
    ...
    headers: { Authorization: `Bearer ${token}`, Accept: 'application/json', 'Content-Type': 'application/json', 'User-Agent': 'code-foundry' },
...
async function cloudflare(env, suffix, body) {
  ...
  const response = await requestJson(
    ...
    required(env, 'CLOUDFLARE_API_TOKEN'), body,
  )
  if (response.success !== true || !Object.hasOwn(response, 'result')) throw new Error('Invalid Cloudflare API envelope')
...
async function github(env, suffix, body) {
  ...
  return requestJson(`https://api.github.com/repos/${repository}/${suffix}`, required(env, 'GH_TOKEN'), body)
}
...
export function latestDeployment(result) {
  ...
  if (deployments.some((item) => !Number.isFinite(Date.parse(item.created_on)))) throw new Error('Deployment timestamp is missing')
...
async function assertCurrent(env, state) {
  ...
  if (env.GITHUB_REF !== `refs/heads/${branch}`) throw new Error('Production delivery requires the default branch')
  const latest = await github(env, `git/ref/heads/${encodeURIComponent(branch)}`)
  if (latest?.object?.sha !== state.sourceSha) throw new Error('A newer source commit exists; refusing stale deployment')
}

/** @param {NodeJS.ProcessEnv} env @param {any} state @param {number} percentage */
async function activate(env, state, percentage) {
  if (state.candidateVerified !== true || !state.bindingInspection) throw new Error('Candidate verification is required before promotion')
  ...
  state.bindingInspection = inspectBindings(version, bindingPolicy(process.cwd(), env.BINDING_POLICY_FILE ?? ''), 'production')
...
async function rollback(env, state) {
  if (env.AUTO_ROLLBACK !== 'true') throw new Error('Automatic rollback is disabled; use the recorded prior versions for manual recovery')
  if (state.bindingInspection?.rollbackSafe !== true) throw new Error('Automatic rollback is restricted to stateless Workers')
  if (!state.cloudflareDeploymentId) throw new Error('This run has not activated a deployment')
  const current = latestDeployment(await cloudflare(env, 'deployments'))
  if (current?.id !== state.cloudflareDeploymentId) throw new Error('Current deployment changed; refusing to roll back another rollout')
  ...
  for (const previous of versions) {
    ...
    const inspection = inspectBindings(version, bindingPolicy(process.cwd(), env.BINDING_POLICY_FILE ?? ''), 'production')
    if (!inspection.stateless) throw new Error('The previous version is stateful; automatic rollback is unsafe')
  }
  const result = await cloudflare(env, 'deployments', {
    strategy: 'percentage', versions,
    annotations: { 'workers/message': `Code Foundry rollback after failed verification of ${state.sourceSha}` },
  })
...
async function verify(env, state) {
  if (!Array.isArray(state.bindingInspection?.bindings)) throw new Error('Binding inspection is required before verification')
  const phase = env.FOUNDRY_PHASE ?? 'candidate'
  if (!['candidate', 'canary', 'production'].includes(phase)) throw new Error('Invalid verification phase')
  ...
  if (!smokePath.startsWith('/') || smokePath.startsWith('//')) throw new Error('smoke-path must be an origin-relative path')
  const target = new URL(smokePath, url)
  if (target.origin !== new URL(url).origin) throw new Error('smoke-path must remain on the deployment origin')
  let healthy = false
  ...
  const childEnv = { ...env, BASE_URL: url, FOUNDRY_DEPLOYMENT_PHASE: phase }
  delete childEnv.CLOUDFLARE_API_TOKEN
  delete childEnv.GH_TOKEN
  delete childEnv.GITHUB_TOKEN
  const result = spawnSync(argv[0], argv.slice(1), { cwd: process.cwd(), stdio: 'inherit', env: childEnv, timeout: 600000 })
  if (result.error) throw result.error
  if (result.status !== 0) throw new Error(`${phase} verification command failed (${result.status ?? result.signal})`)
  state.verifications ??= []
...
export async function deliveryCommand(command, env = process.env) {
  ...
  const state = existsSync(file) ? JSON.parse(readFileSync(file, 'utf8')) : {
    schemaVersion: 1, kind: 'code-foundry-cloudflare-delivery', sourceSha: required(env, 'GITHUB_SHA'),
    worker: required(env, 'FOUNDRY_WORKER'),
  }
  if (!/^[0-9a-f]{40}$/i.test(state.sourceSha) || state.sourceSha !== env.GITHUB_SHA || state.worker !== env.FOUNDRY_WORKER)
    throw new Error('Delivery state does not match the checked-out source and Worker')
  try {
    if (command === 'init-production') {
      if (env.CANDIDATE_VERIFIED !== 'true') throw new Error('Upstream candidate verification did not pass')
      if (env.CANDIDATE_SOURCE_SHA !== state.sourceSha) throw new Error('Candidate source SHA mismatch')
      ...
      if (!/^sha256:[0-9a-f]{64}$/.test(state.artifactDigest)) throw new Error('Invalid build artifact digest')
      state.candidateVerified = true
      state.bindingInspection = {}
    } else if (command === 'upload') {
      if (!['preview', 'production'].includes(env.FOUNDRY_MODE ?? '')) throw new Error('Unsupported delivery mode')
      const event = env.GITHUB_EVENT_NAME
      if (event === 'pull_request_target') throw new Error('Privileged pull_request_target execution is not supported')
      ...
      const result = spawnSync(argv[0], argv.slice(1), {
        ...
        env: { ...env, WRANGLER_OUTPUT_FILE_PATH: outputFile, WRANGLER_SEND_METRICS: 'false', NO_COLOR: '1' },
      })
      if (result.error) throw result.error
      if (result.status !== 0) throw new Error(`Wrangler upload failed (${result.status ?? result.signal})`)
      ...
      state.bindingInspection = inspectBindings(version, bindingPolicy(process.cwd(), env.BINDING_POLICY_FILE ?? ''), mode)
    } else if (command === 'verify') await verify(env, state)
    else if (command === 'assert-current') await assertCurrent(env, state)
    else if (command === 'activate') await activate(env, state, Number(env.DEPLOY_PERCENTAGE ?? '100'))
      ...
      const deployment = await github(env, 'deployments', {
        ref: state.sourceSha, environment: required(env, 'DEPLOYMENT_ENVIRONMENT'),
        auto_merge: false, required_contexts: [], production_environment: production,
        transient_environment: !production, task: 'code-foundry-delivery',
      })
      ...
      await github(env, `deployments/${deployment.id}/statuses`, { state: 'in_progress', auto_inactive: false })
      ...
      await github(env, `deployments/${state.githubDeploymentId}/statuses`, {
        state: status, auto_inactive: false,
        environment_url: env.FOUNDRY_PHASE === 'production' ? httpsUrl(required(env, 'PRODUCTION_URL')) : (state.url ?? ''),
        description: state.rolledBack ? 'Verification failed; stateless rollback completed' : `Code Foundry delivery ${status}`,
      })
...
if (process.argv[1] && pathToFileURL(resolve(process.argv[1])).href === import.meta.url) {
  try { await deliveryCommand(process.argv[2]) }
  catch (error) {
