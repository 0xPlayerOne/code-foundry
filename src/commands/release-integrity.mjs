...
function validateRepository(repository) {
  if (!/^[\w.-]+\/[\w.-]+$/.test(repository) || repository.split('/').some((part) => ['.', '..'].includes(part)))
    throw new Error('repository must be an owner/name on github.com')
}

/** @param {string} tag */
function validateTag(tag) {
  if (!tag || tag.startsWith('-') || /[\x00-\x20\x7f~^:?*\[\\]/.test(tag) || tag.includes('..') || tag.includes('@{'))
...
function json(run, argv) {
  ...
  if (result.status !== 0) throw new Error(`GitHub verification command failed (${result.status ?? 'signal'}); state is unverified`)
  try { return JSON.parse(result.stdout) }
  catch { throw new Error('GitHub verification returned invalid JSON; state is unverified') }
}
...
export function verifyImmutableSetting(repository, run = runGh) {
  ...
  return { schemaVersion: 1, kind: 'code-foundry-release-integrity', status: 'passed',
    repository, check: 'immutable-setting', enforcedByOwner: result.enforced_by_owner === true }
}
...
export function assetDigest(root, file) {
  ...
  if (!rel || rel === '..' || rel.startsWith(`..${sep}`)) throw new Error('Asset path escapes repository')
  let current = base
  ...
  return { file: rel.split(sep).join('/'), name: basename(path), bytes: content.length,
    digest: `sha256:${createHash('sha256').update(content).digest('hex')}`, path }
}
...
export function assetManifest(root, files) {
  ...
  if (new Set(assets.map((asset) => asset.file)).size !== assets.length) throw new Error('Duplicate artifact')
  if (assets.some((asset) => /[\r\n\\]/.test(asset.file))) throw new Error('Unsupported checksum filename')
  return { schemaVersion: 1, kind: 'code-foundry-asset-manifest',
...
function tagCommit(run, repository, tag) {
  ...
  if (object?.type !== 'commit' || !/^[0-9a-f]{40}$/i.test(object.sha ?? '')) throw new Error('Release tag does not resolve to a commit')
  return object.sha
}
...
export function verifyRelease(options, run = runGh) {
  ...
  validateRepository(repository)
  validateTag(tag)
  if (options.expectedSha && !/^[0-9a-f]{40}$/i.test(options.expectedSha)) throw new Error('expected-sha must be a full commit SHA')
  ...
  for (const file of options.assets ?? []) {
    ...
    json(run, ['release', 'verify-asset', tag, asset.path, '--repo', repository, '--format', 'json'])
  ...
  return { schemaVersion: 1, kind: 'code-foundry-release-integrity', status: 'passed',
    check: 'release-attestation', repository, tag, sourceSha, immutable: true, assets }
}
...
export function integrityCommand(argv, run = runGh) {
  ...
  while (args.length) {
    ...
    if (!key || !['--repo', '--tag', '--root', '--asset', '--expected-sha'].includes(key)) throw new Error(`Unknown argument: ${key}`)
  ...
  const allowed = command === 'settings' ? ['--repo'] : command === 'manifest' ? ['--root'] : ['--repo', '--tag', '--root', '--expected-sha']
  if (Object.keys(values).some((key) => !allowed.includes(key)) || (command === 'settings' && assets.length))
  ...
  if (command === 'release') return verifyRelease({ repository: values['--repo'] ?? '', tag: values['--tag'] ?? '',
    root: values['--root'], expectedSha: values['--expected-sha'], assets }, run)
  throw new Error('Use release-integrity settings, release, or manifest')
}

if (process.argv[1] && pathToFileURL(resolve(process.argv[1])).href === import.meta.url) {
  try { console.log(JSON.stringify(integrityCommand(process.argv.slice(2)), null, 2)) }
  catch (error) {
    console.log(JSON.stringify({ schemaVersion: 1, kind: 'code-foundry-release-integrity', status: 'failed',
