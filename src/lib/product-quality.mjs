...
function budget(value, name) {
  requireValue(
    ...
    `${name} must be a finite nonnegative budget`
  )
  return value
}
...
export function checkStaticSite(root, profile) {
  ...
  requireValue(
    Array.isArray(profile.routes) && profile.routes.length > 0,
    'Static routes must be nonempty'
  )
  ...
  for (const route of profile.routes) {
    requireValue(
      typeof route.path === 'string' && route.path.startsWith('/') && !route.path.startsWith('//'),
      'Routes must be origin-relative paths'
    )
    const url = new URL(route.path, origin)
    requireValue(
      url.origin === origin.origin && !url.search && !url.hash && !routes.has(url.pathname),
      'Duplicate or invalid static route'
    )
    const javascriptAssets = route.javascriptAssets ?? []
    requireValue(
      Array.isArray(javascriptAssets) &&
        javascriptAssets.every((asset) => typeof asset === 'string' && asset.length > 0),
      `${url.pathname}: javascriptAssets must be a list of nonempty strings`
    )
    const file = ownedPath(dist, route.html)
    const html = readFileSync(file, 'utf8')
    routes.set(url.pathname, { file, ...readTags(html) })
    routeAssets.set(url.pathname, javascriptAssets)
  }
  ...
  for (const [path, page] of routes) {
    ...
    requireValue(
      !page.tags.some((tag) => tag.name === 'base'),
      `${path}: base elements require a repository-owned checker`
    )
    const title = page.clean.match(/<title(?:\s[^>]*)?>([^<]*)<\/title\s*>/i)?.[1]?.trim()
    requireValue(title, `${path}: missing or empty title`)
    const descriptions = page.tags.filter(
      (tag) => tag.name === 'meta' && tag.attrs.name?.toLowerCase() === 'description'
    )
    requireValue(
      descriptions.length === 1 && descriptions[0].attrs.content?.trim(),
      `${path}: expected one nonempty meta description`
    )
    const canonicals = page.tags.filter(
      (tag) =>
        tag.name === 'link' && tag.attrs.rel?.toLowerCase().split(/\s+/).includes('canonical')
    )
    requireValue(
      canonicals.length === 1 && canonicals[0].attrs.href === canonical,
      `${path}: canonical URL mismatch`
    )
    requireValue(
      !page.tags.some(
        (tag) =>
          tag.name === 'meta' &&
          ['robots', 'googlebot'].includes(tag.attrs.name?.toLowerCase()) &&
          /(?:^|[,\s])noindex(?:$|[,\s])/i.test(tag.attrs.content ?? '')
      ),
      `${path}: unexpected noindex`
    )
    const scripts = new Set(routeAssets.get(path))
    const images = new Set()
    for (const tag of page.tags) {
      if (tag.name === 'script' && tag.attrs.src) scripts.add(tag.attrs.src)
      if (tag.name === 'img' && tag.attrs.src && !tag.attrs.src.startsWith('data:'))
        images.add(tag.attrs.src)
      if (['img', 'source'].includes(tag.name) && tag.attrs.srcset) {
        requireValue(
          !tag.attrs.srcset.includes('data:'),
          'Data-URL srcsets require a repository-owned checker'
        )
        for (const candidate of tag.attrs.srcset.split(',')) {
          const resource = candidate.trim().split(/\s+/)[0]
          requireValue(resource, `${path}: srcset contains an empty candidate`)
          images.add(resource)
        }
      }
      ...
      if (targetPage) {
        if (target.hash) {
          ...
          requireValue(
            targetPage.tags.some(
              (entry) => entry.attrs.id === id || (entry.name === 'a' && entry.attrs.name === id)
            ),
            `${path}: missing anchor ${target.hash}`
          )
        }
      } else {
        const asset = ownedPath(dist, decodeURIComponent(target.pathname).replace(/^\//, ''))
        requireValue(
          existsSync(asset) && lstatSync(asset).isFile(),
          `${path}: broken internal link ${target.pathname}`
        )
      }
    }
    const assetBytes = (/** @type {Set<string>} */ assets) =>
      [...assets].reduce((sum, resource) => {
        const url = new URL(resource, canonical)
        requireValue(
          url.origin === origin.origin,
          `${path}: external resource needs a measured repository-owned budget: ${url.origin}`
        )
    ...
    const result = {
      ...
      javascriptBytes: assetBytes(scripts) + inlineBytes,
      imageBytes: assetBytes(images),
    }
    requireValue(
      result.htmlBytes <= htmlBudget &&
        result.javascriptBytes <= scriptBudget &&
        result.imageBytes <= imageBudget,
      `${path}: resource budget exceeded`
    )
    metrics.push(result)
  }
  if (profile.sitemap) {
    ...
    const locations = new Set(
      [
        ...sitemapWithoutComments.matchAll(
          /<url\b[^>]*>[\s\S]*?<loc>\s*([^<]+)\s*<\/loc>[\s\S]*?<\/url\s*>/gi
        ),
      ].map((match) => decode(match[1].trim()))
    )
    for (const path of routes.keys())
      requireValue(locations.has(new URL(path, origin).href), `${path}: absent from sitemap`)
    const robots = readFileSync(ownedPath(dist, profile.robots ?? 'robots.txt'), 'utf8')
    requireValue(
      robots
        .split(/\r?\n/)
        .some((line) => line.trim() === `Sitemap: ${new URL(profile.sitemap, origin).href}`),
      'robots.txt must reference the checked sitemap'
    )
    requireValue(!/^Disallow:\s*\/\s*$/im.test(robots), 'robots.txt blocks the entire site')
  }
  if (profile.redirects) {
    requireValue(
      Array.isArray(profile.redirects.rules) && profile.redirects.rules.length > 0,
      'Redirect expectations must be nonempty'
    )
    const lines = readFileSync(ownedPath(dist, profile.redirects.file), 'utf8')
      .split(/\r?\n/)
      .map((line) => line.trim())
    ...
    for (const rule of profile.redirects.rules) {
      requireValue(
        typeof rule.from === 'string' &&
          ...
          [301, 302, 303, 307, 308].includes(rule.status),
        'Invalid redirect expectation'
      )
      ...
      requireValue(
        matches.length === 1 &&
          matches[0].split(/\s+/).join(' ') === `${rule.from} ${rule.to} ${rule.status}`,
        `Missing or ambiguous redirect: ${rule.from}`
      )
    }
  }
  return metrics
...
export function runQualityCommand(argv, root, output, timeout = 120_000) {
  ...
  const result = spawnSync(args[0], args.slice(1), {
    ...
    env: { ...process.env, CI: 'true', CODE_FOUNDRY_QUALITY_OUTPUT_DIR: output },
  })
  if (result.stdout) process.stderr.write(result.stdout)
  if (result.stderr) process.stderr.write(result.stderr)
  requireValue(
    !result.error && result.status === 0,
    `Quality command failed: ${args[0]} (${result.status ?? result.error?.message})`
  )
...
export function checkWorker(root, profile, output) {
  requireValue(
    ...
    'Worker bundle files are required'
  )
  ...
  requireValue(
    rawBytes <= budget(profile.budgets?.rawBytes, 'rawBytes'),
    'Worker raw-byte budget exceeded'
  )
  requireValue(
    gzipBytes <= budget(profile.budgets?.gzipBytes, 'gzipBytes'),
    'Worker gzip budget exceeded'
  )
...
export function checkPackage(root, profile, output) {
  ...
  requireValue(
    Array.isArray(profile.imports) &&
      profile.imports.length > 0 &&
      profile.imports.every(
        (/** @type {unknown} */ value) =>
          typeof value === 'string' &&
          (value === manifest.name || value.startsWith(`${manifest.name}/`))
      ),
    'Imports must exercise the candidate package exports'
  )
  const runtimes = profile.runtimes ?? ['node']
  requireValue(
    Array.isArray(runtimes) &&
      runtimes.length > 0 &&
      runtimes.every((/** @type {unknown} */ value) =>
        ['node', 'bun'].includes(/** @type {string} */ (value))
      ),
    'Package runtimes must be a nonempty node/bun list'
  )
  ...
  try {
    ...
    requireValue(
      Array.isArray(pack) && pack.length === 1 && /^[A-Za-z0-9._-]+\.tgz$/.test(pack[0].filename),
      'Expected one package archive'
    )
    requireValue(
      pack[0].files.every(
        (/** @type {{path:string}} */ file) => !/^\.code-foundry(?:\/|$)/.test(file.path)
      ),
      'Exclude .code-foundry quality evidence from the published package'
    )
    ...
    for (const runtime of runtimes)
      runQualityCommand(
        [runtime === 'node' ? process.execPath : 'bun', 'smoke.mjs'],
        consumer,
        output
      )
    requireValue(
      createHash('sha256').update(readFileSync(archive)).digest('hex') === archiveSha256,
      'Package archive changed during compatibility testing'
    )
    return {
      name: manifest.name,
...
export function browserFiles(root, profile, output) {
  ...
  requireValue(
    ['http:', 'https:'].includes(url.protocol) &&
      ...
      !url.search &&
      !url.hash,
    'browser baseURL must be an HTTP(S) origin without credentials or a path'
  )
  requireValue(
    profile.allowRemote === true || ['localhost', '127.0.0.1', '[::1]'].includes(url.hostname),
    'Remote browser checks require allowRemote: true'
  )
  ...
  const routes = profile.routes.map((/** @type {Profile} */ route) => {
    requireValue(
      typeof route.id === 'string' && /^[a-z0-9-]+$/.test(route.id),
      'Route IDs must be safe snapshot names'
    )
    requireValue(
      typeof route.path === 'string' &&
        route.path.startsWith('/') &&
        new URL(route.path, url).origin === url.origin,
      'Browser routes must stay on the selected origin'
    )
    requireValue(
      typeof route.readySelector === 'string' && route.readySelector.length > 0,
      'A readySelector is required for each browser route'
    )
    return route
  })
  requireValue(
    new Set(routes.map((/** @type {Profile} */ route) => route.id)).size === routes.length,
    'Duplicate browser route IDs'
  )
  if (profile.webServerCommand !== undefined)
    requireValue(
      typeof profile.webServerCommand === 'string' && profile.webServerCommand.length > 0,
      'webServerCommand must be a nonempty command string'
    )
  ...
  const config = {
    ...
    ...(profile.webServerCommand
      ? {
          webServer: {
            command: profile.webServerCommand,
            cwd: root,
            url: profile.baseURL,
...
export function checkBrowser(root, profile, output) {
  ...
  writeFileSync(
    join(output, 'playwright.config.mjs'),
    `export default ${JSON.stringify(config, null, 2)}\n`
  )
  runQualityCommand(
    [process.execPath, cli, 'test', '--config', join(output, 'playwright.config.mjs')],
    root,
    output,
    600_000
  )
  ...
  validateBrowserReport(
    report,
    profile.routes.map((/** @type {Profile} */ route) => `quality: ${route.id}`)
  )
...
export function validateBrowserReport(report, expectedRoutes) {
  ...
  requireValue(
    report.stats?.unexpected === 0 &&
      ...
      report.stats?.expected > expectedRoutes.length,
    'Browser checks need passing routes and at least one journey, without skips or flakes'
  )
  requireValue(
    specs.length > expectedRoutes.length,
    'Browser report has no repository-owned journey'
  )
  for (const title of expectedRoutes)
    requireValue(
      specs.filter((spec) => spec.title === title).length === 1,
      `Missing or duplicate browser route: ${title}`
    )
  for (const spec of specs) {
    requireValue(
      spec.ok === true && Array.isArray(spec.tests) && spec.tests.length > 0,
      'Browser specification did not pass'
    )
    for (const item of spec.tests)
      requireValue(
        item.expectedStatus === 'passed' &&
          ...
          item.results.every((/** @type {Profile} */ value) => value.status === 'passed'),
        'Expected failures and incomplete browser results are not accepted'
      )
  }
}
...
export function runProductQuality(root, config, phase = 'build') {
  requireValue(
    config.schema_version === 1 && Array.isArray(config.profiles) && config.profiles.length > 0,
    'Expected a version-1 quality manifest with profiles'
  )
  ...
  requireValue(
    ids.every((/** @type {unknown} */ id) => typeof id === 'string' && /^[a-z0-9-]+$/.test(id)) &&
      new Set(ids).size === ids.length,
    'Profile IDs must be unique safe names'
  )
  for (const profile of config.profiles) {
    requireValue(
      ['static-site', 'web-app', 'worker', 'package'].includes(profile.type),
      `Unknown quality profile: ${profile.type}`
    )
    requireValue(
      ['build', 'browser', 'deployed'].includes(
        profile.phase ?? (profile.type === 'web-app' ? 'browser' : 'build')
      ),
  ...
  const profiles = config.profiles.filter(
    (/** @type {Profile} */ profile) =>
      (profile.phase ?? (profile.type === 'web-app' ? 'browser' : 'build')) === phase
  )
