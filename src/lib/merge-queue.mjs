...
export function renderMergeQueueCaller(config, ref) {
  ...
  ensure(/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(repository) && !repository.split('/').some((part) => ['.', '..'].includes(part)), 'Invalid queue runtime repository')
  ensure(/^(?:[a-f0-9]{40}|v\d+\.\d+\.\d+(?:-[A-Za-z0-9.-]+)?)$/.test(ref), 'Merge queues require a released version or exact commit pin, not a moving branch')
  ...
  ensure(features.some((feature) => ['all', 'validation', 'ci', 'test', 'security', 'codeql'].includes(feature)), 'Merge queues require the canonical PR validation caller')
  ...
  const shards = JSON.parse(config.codeql_rust_shards || '["all"]')
  ensure(Array.isArray(shards) && shards.length > 0 && shards.every((value) => typeof value === 'string' && /^[A-Za-z0-9_./*-]+$/.test(value)), 'Invalid queue Rust shards')
  const positive = (/** @type {string} */ value) => {
    ensure(/^[1-9]\d*$/.test(value) && Number.isSafeInteger(Number(value)), 'Queue Rust concurrency must be a positive integer')
    return value
  }
  const threads = positive(config.codeql_rust_threads || '1')
  const parallel = positive(config.codeql_rust_max_parallel || '1')
...
export function validateMergeGroup(payload, environment, branches) {
  ensure(Array.isArray(branches) && branches.length > 0 && branches.every((branch) => ['main', 'staging'].includes(branch)) && new Set(branches).size === branches.length, 'Invalid allowed merge-queue branches')
  ensure(environment.GITHUB_EVENT_NAME === 'merge_group' && payload.action === 'checks_requested', 'Only merge_group checks_requested is accepted')
  const group = payload.merge_group
  ensure(group && /^[a-f0-9]{40}$/.test(group.head_sha ?? '') && /^[a-f0-9]{40}$/.test(group.base_sha ?? ''), 'Merge-group SHA identity is incomplete')
  ensure(branches.some((branch) => group.base_ref === `refs/heads/${branch}`), 'Merge-group base branch is not enabled')
  const branch = group.base_ref.slice('refs/heads/'.length)
  ensure(typeof group.head_ref === 'string' && group.head_ref.startsWith(`refs/heads/gh-readonly-queue/${branch}/`) && group.head_ref.length > `refs/heads/gh-readonly-queue/${branch}/`.length, 'Unexpected merge-group ref')
  ensure(environment.GITHUB_SHA === group.head_sha && environment.GITHUB_REF === group.head_ref, 'Workflow commit/ref differs from the merge group')
  ensure(typeof environment.GITHUB_REPOSITORY === 'string' && environment.GITHUB_REPOSITORY === payload.repository?.full_name, 'Merge-group repository identity mismatch')
  return { mode: 'audit', head_sha: group.head_sha, base_sha: group.base_sha, base_ref: group.base_ref }
}
...
export function inspectQueueCaller(root, content) {
  ...
  let existing = null
  try { existing = readFileSync(current, 'utf8') } catch (error) {
    if (/** @type {NodeJS.ErrnoException} */ (error).code !== 'ENOENT') throw error
  }
  if (existing !== null && !existing.startsWith(QUEUE_MARKER)) {
    ensure(content === null, 'A repository-owned merge-queue caller already exists; refusing to overwrite it')
...
if (process.argv[1] && pathToFileURL(resolve(process.argv[1])).href === import.meta.url) {
  try {
    ...
    console.log(JSON.stringify(validateMergeGroup(JSON.parse(readFileSync(process.env.GITHUB_EVENT_PATH, 'utf8')), process.env, JSON.parse(process.env.FOUNDRY_QUEUE_BRANCHES || '[]'))))
  } catch (error) {
