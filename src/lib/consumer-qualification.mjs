...
export function seedFixture(root, fixture) {
  ...
  writeFileSync(join(root, '.github/workflows/custom.yml'), 'name: Custom\non: workflow_dispatch\njobs:\n  check:\n    runs-on: ubuntu-latest\n    steps:\n      - run: echo preserved\n')
  writeFileSync(join(root, 'LICENSE'), 'Consumer-owned license\n')
  if (fixture.manager) {
    writeFileSync(join(root, 'package.json'), JSON.stringify({
      name: `qualification-${fixture.name}`, version: '0.0.0', private: true,
    ...
    writeFileSync(join(root, fixture.lock), locks[/** @type {keyof typeof locks} */ (fixture.manager)])
  } else if (fixture.name === 'nested-rust') {
    mkdirSync(join(root, 'native/core/src'), { recursive: true })
    writeFileSync(join(root, 'native/core/Cargo.toml'), '[package]\nname = "fixture"\nversion = "0.1.0"\nedition = "2021"\n')
...
export function localizeCalls(text, repository, ref) {
  ...
  const pattern = new RegExp(`(uses:\\s*)${escape(repository)}/\\.github/workflows/([A-Za-z0-9_.-]+)@${escape(ref)}(?=\\s|$)`, 'g')
...
function lintContracts(consumer, candidate, destination, actionlint, version) {
  ...
  const workflows = readdirSync(join(destination, '.github/workflows')).filter((name) => /\.ya?ml$/.test(name))
  run(actionlint, ['-shellcheck=', '-pyflakes=', ...workflows.map((name) => join(destination, '.github/workflows', name))], destination)
  return paths.length
}
...
export function qualifyCandidate(options) {
  if (!/^[a-f0-9]{40}$/.test(options.sourceSha)) throw new Error('An exact 40-character source SHA is required')
  ...
  const report = {
    schema_version: 1, source_sha: options.sourceSha, artifact_sha256: before,
    node: process.version, complete: false, actionlint: Boolean(options.actionlint),
  ...
  try {
    ...
    run('npm', ['install', '--offline', '--ignore-scripts', '--no-audit', '--no-fund', '--package-lock=false', packageFile], host)
    ...
    for (const fixture of FIXTURES) {
      ...
      assert.ok(readFileSync(join(root, 'AGENTS.md'), 'utf8').startsWith('# Consumer-owned instructions\n\nPreserve this paragraph exactly.\n'))
      ...
      if (fixture.manager) assert.match(readFileSync(join(root, '.github/code-foundry.yml'), 'utf8'), new RegExp(`^package_manager: ${fixture.manager}$`, 'm'))
      ...
      if (options.actionlint) lintContracts(root, candidate, join(temporary, `lint-${fixture.name}`), options.actionlint, manifest.version)
...
if (process.argv[1] && resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  try {
    ...
    if (!packageFile || !sourceSha || !reportPath || extra.length) throw new Error('Usage: node consumer-qualification.mjs PACKAGE.tgz SOURCE_SHA REPORT.json [ACTIONLINT]')
    console.log(JSON.stringify(qualifyCandidate({ packageFile, sourceSha, reportPath, actionlint })))
  } catch (error) {
