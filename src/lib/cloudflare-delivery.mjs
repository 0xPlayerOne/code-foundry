...
export function parseUpload(content, worker) {
  ...
  if (!new URL(url).hostname.endsWith('.workers.dev'))
    throw new Error('Expected a Workers version preview URL')
...
export function digestBuild(root, directory) {
  ...
  for (const part of location.split(sep)) {
    ...
    if (lstatSync(ancestor).isSymbolicLink())
      throw new Error('Build tree must not contain symlinks')
  }
  ...
  function visit(currentDirectory) {
    ...
    for (const name of entries) {
      const file = resolve(currentDirectory, name)
      ...
      if (stat.isDirectory()) visit(file)
        ...
        hash
          .update(`${relative(target, file).split(sep).join('/')}\0${content.length}\0`)
          .update(content)
        files += 1
...
export function inspectBindings(version, policy, mode) {
  ...
  if (
    Object.values(version?.resources?.script_runtime?.exports ?? {}).some(
      (entry) => /** @type {any} */ (entry)?.type === 'durable-object'
    )
  )
    throw new Error(
      'Durable Object exports require a repository-specific preview and migration workflow'
    )
  ...
  if (
    !Array.isArray(policy.readOnlyBindings ?? []) ||
    !Array.isArray(policy.isolatedBindings ?? [])
  )
  ...
  for (const binding of bindings) {
    if (typeof binding.name !== 'string' || typeof binding.type !== 'string')
      throw new Error('Malformed binding metadata')
    if (binding.type === 'durable_object_namespace')
      throw new Error(
        'Durable Objects require a repository-specific preview and migration workflow'
      )
    ...
    stateless = false
    const isolated = (policy.isolatedBindings ?? []).find(
      (/** @type {any} */ entry) => entry.name === binding.name && entry.type === binding.type
    )
    if (isolated) {
      if (mode !== 'preview')
        throw new Error('Isolated preview bindings must never be promoted into production')
      ...
      if (
        ...
        keys.some(
          (key) =>
            ...
            typeof production[key] !== 'string'
        )
      )
        throw new Error('Isolated bindings require expected and production resource identifiers')
      if (!keys.some((key) => expected[key] !== production[key]))
        throw new Error('Preview binding reuses production resource identifiers')
      if (keys.some((key) => binding[key] !== expected[key]))
        throw new Error(`Isolated resource mismatch for ${binding.name}`)
      result.push({ name: binding.name, type: binding.type, policy: 'isolated' })
      continue
    }
    const reviewed = (policy.readOnlyBindings ?? []).some(
      (/** @type {any} */ entry) => entry.name === binding.name && entry.type === binding.type
    )
...
export function deploymentVersions(candidate, percentage, previous = []) {
  ...
  if (
    ...
    throw new Error('Canary percentage must be greater than zero and at most 100')
  ...
  return [
    { version_id: candidate, percentage },
    { version_id: baseline, percentage: 100 - percentage },
  ]
}
...
export function bindingPolicy(root, file) {
  ...
  if (location === '..' || location.startsWith(`..${sep}`))
    throw new Error('Binding policy escapes checkout')
