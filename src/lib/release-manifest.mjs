  ...
  walk(root, (file) => {
    ...
    if (name === 'package.json') {
      ...
      packages.push({
        ...
        extraFiles: detectExtraFiles(root, directory),
      })
      ...
      packages.push({
        ...
        manifest: file,
        releaseType: 'rust',
        packageName: tomlPackageName(join(root, file)),
        extraFiles: detectExtraFiles(root, directory),
      })
      ...
      packages.push({
        ...
        manifest: file,
        releaseType: 'python',
        packageName: pyprojectName(join(root, file)),
        extraFiles: detectExtraFiles(root, directory),
      })
    }
  })
...
export function buildReleaseConfig(root, existing = {}) {
  ...
  if (packages.length === 1 && packages[0].directory === '.') {
    ...
    for (const file of packages[0].extraFiles) {
      if (!extra.some((entry) => (typeof entry === 'string' ? entry : entry?.path) === file))
        extra.push(file)
    }
    return result
  }
  ...
  for (const entry of packages) {
    ...
    for (const file of entry.extraFiles)
      if (!extra.some((item) => (typeof item === 'string' ? item : item?.path) === file))
        extra.push(file)
...
export function validateReleaseConfig(root, config) {
  ...
  if (detected.length > 1 && !configured)
    return [
      'mixed-language or multi-package repositories require release-please packages configuration',
    ]
  ...
  for (const entry of detected) {
    ...
    else if (item['release-type'] !== entry.releaseType)
      errors.push(`${entry.directory} release-type should be ${entry.releaseType}`)
  }
  return errors
}
